package{
	...
	class lessonSetup extends MovieClip{
		...
		private var _numOfQuestions;
		
		//Lesson resources
		private var _hiragana;
		private var _katakana;
		private var _testSet;
		private var _lessonXML:XML = <chars> </chars>;
		
		
		public function lessonSetup(){
			//Make Background
			...
			this.addEventListener("BackToOptions",function(){
														 makeOptionsScreen();
														 });
		}
		
		private function makeOptionsScreen(){
			if(_lesson){
				removeChild(_lesson);
			}
		...
		private function setupLesson(options:Array){
			...
			_useYoon= options[7];
			_numOfQuestions = options[8];
			
			//init lesson objects
			_hiragana = new HiraganaClean();
		...
		private function getHiraGroups(){
			for(var i=0;i<_hiragana.group.length();i++){
				for each(var item in _charInclude){
					...
					_lessonXML.appendChild(_hiragana.group[i]);
					}
				}
			}
		}
		...
		private function getKataGroups(){
			for(var i=0;i<_katakana.group.length();i++){
				for each(var item in _charInclude){
					...
					_lessonXML.appendChild(_katakana.group[i]);
					}
				}
			}
		}
			...
			myTimer.addEventListener("timer", 
				 function(){
					if(count > 1){
						...
						Tweener.addTween(countTxt,{scaleX:1,scaleY:1,time:.5});
						Tweener.addTween(countTxt,{alpha:0,time:1.5,onComplete:function(){removeChild(countTxt)}}); 
						countTxt.text = 'GO!!';
						myTimer.stop();
						startLesson();
					}
		...
		private function startLesson(){
			_lesson = new randCharLesson(_lessonXML,_numOfQuestions);
			addChild(_lesson);
